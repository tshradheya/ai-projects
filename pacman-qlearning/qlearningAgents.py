...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if legalActions and len(legalActions) > 0:
            for act in legalActions:
                values_list.append(self.getQValue(state, act))
    ...
    def computeActionFromQValues(self, state):
        ...
        if legalActions and len(legalActions) > 0:
            max_qValue = self.getQValue(state, legalActions[0])
            best_action_list.append(legalActions[0])

            for act in legalActions:
                val = self.getQValue(state, act)
                if val > max_qValue:
                    best_action_list = list()
                    best_action_list.append(act)
                    max_qValue = val
                elif val == max_qValue:
                    best_action_list.append(act)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature, featureWeight in features.items():
            qVal += (self.weights[feature] * featureWeight)
    ...
    def update(self, state, action, nextState, reward):
        ...
        for feature, featureWeight in features.items():
            self.weights[feature] += (self.alpha * diff * featureWeight)
